...
#             # self.encoders.append(AutoEncoder(input_dim[v], high_feature_dim, dims).to(device))
#             # self.decoders.append(AutoDecoder(input_dim[v], high_feature_dim, dims).to(device))
...
#             # nn.Linear((high_feature_dim + high_feature_dim), low_feature_dim),
#
...
class Network(nn.Module):
    ...
    def forward(self, xs):
        ...
        for v in range(self.views):
            x = xs[v]
            z = self.encoders[v](x)
            # h1 = normalize(self.instance_head2(z), dim=1)
            # 细粒度特征和粗粒度特征融合
            h1 = normalize(self.instance_head(z), dim=1)
